...
the full signature of the real one, `readFile` overloads included, and the class
...
a bundle discovers that local files are unavailable at the point it tries to
read one, with an error naming the path
